...
local uservolume = CreateClientConVar("srp_wowozela_volume", "1", true)
wowozela = {}
...
do -- sample meta
	...
	function META:SetSample(i, path)
		...
		self.CSP[i]:SetSoundLevel(60)
	end

	function META:ChangeVolume(i, num)
		if self.CSP[i] then
			self.CSP[i]:ChangeVolume(self.Volume * uservolume:GetFloat(), -1)
		end
	end
	...
	function META:Start(i, id)
		...
		if self.CSP[i] and dist < 62500 then
			if id then
				...
				snd = self.CSP[i]
				snd:PlayEx(self.Volume * (1 - (dist / 62500)) * uservolume:GetFloat(), self.Pitch)
				self.IDs[id] = snd
			else
				self.CSP[i]:PlayEx(self.Volume * (1 - (dist / 62500)) * uservolume:GetFloat(), self.Pitch)
			end
		end
	end
	...
	function META:Think()
		...
		if (self:IsKeyDown(IN_ATTACK) or self:IsKeyDown(IN_ATTACK2)) and CurTime() > self.nextParticle then
			self:MakeParticle()
