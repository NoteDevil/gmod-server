...
OS:NewCommand(":gnet", function(client, entity, arguments)
	if ( !arguments[1] ) then
		...
		gTerminal:Broadcast(entity, "    :gnet c <network> - Creates a network.");
	...
	elseif (arguments[1] == "c") then
		...
		if (entity.networkID) then
			gTerminal:Broadcast(entity, "You are currently hosting Network '"..entity.networkID.."'", GT_COL_WRN);
		...
		for k, v in pairs( ents.FindByClass(entity.ClassName) ) do
			if (v.isHost and v.networkID == arguments[2]) then
				gTerminal:Broadcast(entity, "Network ID already taken!", GT_COL_ERR);
		...
		if (entity.netPassword) then
			gTerminal:Broadcast(entity, "Created Network '"..entity.networkID.."' with password '"..entity.netPassword.."'", GT_COL_SUCC);
		else
			gTerminal:Broadcast(entity, "Created Network '"..entity.networkID.."'", GT_COL_SUCC);
		end;
