...
-- generate main frame
hook.Add("PlayerIsLoaded", "cats", function()
...
net.Receive('cats.getAdminData', function()
    ...
    -- steamid
    local l_steamid = vgui.Create('DLabel', p)
    l_steamid:DockMargin(20,5,20,0)
    l_steamid:Dock(TOP)
    l_steamid:SetTall(20)
    l_steamid:SetFont('cats.medium')
    l_steamid:SetText(data.steamID)
    l_steamid:SetToolTip('Click to copy SteamID')
    l_steamid.DoClick = function(self) SetClipboardText(data.steamID) end

    -- stats
    local stats = {
        { 'ОЦЕНКА', '★' .. math.Round(data.ratingTotal, 2) },
        { 'ЖАЛОБЫ', data.claimsTotal },
        { 'ПОЛЬЗОВАТЕЛИ', data.uniqueUsers },
        { 'ЖАЛОБ В НЕДЕЛЮ', math.Round(data.claimsTotal / (data.updateTime - data.createdTime) * 604800) },
        { 'ВРЕМЯ ЖАЛОБ', niceTime(data.ticketTimeTotal) },
    }

    if data.playTimeTotal then
        table.insert( stats, { 'ВРЕМЯ ИГРЫ', niceTime(data.playTimeTotal) })
    end
    ...
    for i, v in ipairs(stats) do
        ...
        item.Paint = function(self, w, h)
            ...
            draw.SimpleText(v[1], "cats.medium", w/2, 30, Color(220,220,220), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
            draw.SimpleText(v[2], "cats.xlarge", w/2, 65, Color(220,220,220), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        end
    ...
    l_timeCard:SetText('Карта времени игры')
