...
--	resource.AddWorkshop("411393143")
	
	hook.Add("InitPostEntity","AddLocksToDoors",function()
	timer.Simple(5,function()
			for i,v in pairs(ents.GetAll()) do 
			if IsValid(v) and v:GetClass() == "prop_door_rotating" and (v:getKeysDoorGroup() != nil or v:getKeysDoorTeams() != nil) then
				local lock = ents.Create(DefaultLockOnDoors)
				lock:Spawn()
				lock.Owner = v 
				lock:SetPos(v:GetPos())
				lock:ParentTo(v,true)
				v:keysUnLock()			 
			end	
		end
	end)   
	...
	concommand.Add("DemontageLock",function(ply)
		if IsValid(ply) and ply:Alive() then
			local ent = ply:GetEyeTrace().Entity
			if not IsValid(ent) or not ent:isKeysOwnable() or ent:GetPos():Distance(ply:GetPos()) > 200 then return end
			if ply == ent:getDoorOwner() and IsValid(ent.HaveLock) then
				ent:keysUnLock()
				ent.HaveLock:Remove()
			end
		end	
...
hook.Add("canKeysLock","OMGCustomKeys", function(self, ent)

if not IsValid(ent.HaveLock) and ent:getKeysDoorGroup() == nil and ent:getKeysDoorTeams() == nil and ent:GetClass() == "prop_door_rotating" then return false end

end)


hook.Add("playerBuyDoor","OmgCustomBuyDoors",function(ply, ent)

	timer.Simple(.3,function()
		if not IsValid(ent) or not IsValid(ply) then return end
		if ent:GetClass() == "prop_door_rotating" then
			local lock = ents.Create(DefaultPublicLock)
			lock:Spawn()
			lock.Owner = ply
			lock:SetPos(ent:GetPos())
			lock:ParentTo(ent,nil,true)
			ent:keysUnLock()
		end
	end)
