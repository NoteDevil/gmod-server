...
local use_os = CreateClientConVar("webbrowser_use_os", "0", true)
local webbrowser_enabled = CreateClientConVar("webbrowser_enabled", "1", true)

local mediaurls = {"mp4"} -- File extensions that shouldn't be opened with ingame browser.
gui.OLDOpenURLX = gui.OLDOpenURLX or gui.OpenURL
function gui.OpenURLX(url,useold)
	local extension = url:match(".+%.(.-)%s*$")
	
	if not webbrowser_enabled:GetBool() or useold or table.HasValue(mediaurls, extension) then
		return gui.OLDOpenURLX(url)
	end
