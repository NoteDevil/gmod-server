...
-----------------------------------------------------
-- hook.Add("PlayerIsLoaded", "SRP_Weapon_Selector", function()

local enabled_weaponselector = CreateClientConVar("srp_hud_weapon", "1", true)

local function SRPWeaponSelector()

local scale = (ScrW() / 175 >= 6 and 1) or 0.8

surface.CreateFont("srp_wepsel", {
    size = 20 * scale,
    weight = 300 * scale, 
    antialias = true,
    extended = true,
...
surface.CreateFont("srp_wepsel_small", {
    size = 20 * 0.7 * scale,
    weight = 300 * scale,
    antialias = true,
    extended = true,
...
local WeaponHistory = {}
local ActiveWeapon 
hook.Add("Think", "srp_wepsel", function()
...
hook.Add("CreateMove", "srp_wepsel", function(cmd)
    if newinv then
...
hook.Add("HUDPaint", "srp_wepsel", function()
    ...
    for i, v in pairs(loadout) do
        ...
        draw.SRPBackGround(0, offx, offy, height, height)
        ...
        for j, wep in pairs(v) do
            ...
            draw.SRPBackGroundColored(4, offx, offy, width, height, selected and color_bright or color_dark)

            surface.SetFont("srp_wepsel")
            local w, h = surface.GetTextSize(wep.name)
            if w > width then
                surface.SetFont("srp_wepsel_small")
...
hook.Add("HUDShouldDraw", "srp_wepsel", function(name)
...
hook.Add("PlayerIsLoaded", "SRP_Weapon_Selector", SRPWeaponSelector)
SRPWeaponSelector()