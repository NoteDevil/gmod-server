...
function GM:CanChangeRPName(ply, RPname)
    ...
    if len > 100 then return false, "Too long." end
    if len < 6 then return false, "Too short." end
end
...
local function IsInRoom(listener, talker) -- IsInRoom function to see if the player is in the same room.
    ...
    -- uhh, we can sacrifice this for performance, who cares anyway? it's a schoolrp
    return true
end
