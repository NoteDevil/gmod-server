...
local effectfiles = file.Find( "entities/srp_skateboard/effects/*.lua", "LUA" )
...
function ENT:CreateAvatar()
	...
	self.Avatar = ents.Create( "srp_skateboard_avatar" )
	...
	if( IsValid( pl ) ) then
		...
		if( !IsValid( self.Hull ) ) then
			...
			if ( IsValid( boardphys ) ) then
				...
				-- make
				self.Hull = ents.Create( "srp_skateboard_hull" )
...
local function SetPlayerAnimation( pl, anim )
	...
	if ( !IsValid( board ) || board:GetClass() != "srp_skateboard" ) then return end
...
function ENT:PhysicsCollide( data, physobj )
	...
	if IsValid(data.HitEntity) and data.HitEntity:IsPlayer() and data.HitNormal.z > 0.1 and not IsValid(self:GetDriver()) then
		...
		timer.Simple(0, function()
			self:SetDriver( data.HitEntity ) -- Set the driver
		end)
	end
...
function ENT:PhysicsSimulate( phys, deltatime )
	...
	if ( IsValid( driver ) ) then
		...
		if ( self.Jumped ) then
			...
			jump_power = math.Clamp( jump_power * speed, 170, 300 ) / 5 * up.z
...
function ENT:GiveSWEP( ply )
	...
	if ply:HasWeapon('srp_skate') then
		DarkRP.notify(ply, NOTIFY_ERROR, 3, 'У тебя уже есть скейтборд')
		return
	end

	local wep = ply:Give('srp_skate')
	wep:SetSkateData({
		Model = self:GetModel(),
		Speed = self:GetSpeed(),
		YawSpeed = self:GetYawSpeed(),
		TurnSpeed = self:GetTurnSpeed(),
		PitchSpeed = self:GetPitchSpeed(),
		RollSpeed = self:GetRollSpeed(),
...
hook.Add( "EntityTakeDamage", "Skateboard_EntityTakeDamage", function( ent, dmginfo )
	...
	if ( IsValid( attacker ) ) then
		...
		if ( attacker:GetClass() == "srp_skateboard" ) then
	
			-- get driver
			driver = attacker:GetDriver()
			
		elseif ( attacker:GetClass() == "srp_skateboard_hull" ) then
...
hook.Add("PlayerDisconnected", "SRP_Skateboards", function(ply)

	for k, skate in pairs(ents.FindByClass('srp_skateboard')) do
		if skate:Getowning_ent() == ply then
			skate:Remove()
		end
	end
