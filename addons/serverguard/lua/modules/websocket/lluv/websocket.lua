...
local uv         = lluv
local ut         = lluv.utils
local tools      = include "websocket/tools.lua"
local frame      = include "websocket/frame.lua"
local handshake  = include "websocket/handshake.lua"
local WSError    = include "websocket/error.lua"
...
function WSSocket:__init(opt, s)
  ...
  if opt.utf8 then
    if opt.utf8 == true then
      self._validator = include "websocket/utf8.lua".validator()
    else
