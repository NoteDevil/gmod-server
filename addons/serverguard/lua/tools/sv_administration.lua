...
local function CheckPlayerBanned(communityID, ip, serverPassword, enteredPassword, name)
	...
	if (data) then
		...
		if (endTime > 0 and endTime <= os.time()) then
			...
			-- sorry I changed function below anyway so who the fuck cares
			if (endTime <= 0) then
				return true, string.format("You are permanently banned.\n\nAdmin: %s\nReason: %s", data.admin, data.reason);
			end;
			
			return true, string.format("You have been banned for %s.\n\nAdmin: %s\nReason: %s\n\nYou will be unbanned in %s.", initialText, data.admin, data.reason, text);
		end;
	else
		...
		for steamid, info in pairs(serverguard.banTable) do
			if (info.ip != "" and info.ip == ip) then
				...
				return true, "Banned: "..info.reason;
			end;
		end;
	end;
end;
...
function serverguard:BanPlayer(admin, player, length, reason, bKick, bSilent, adminNameOverride)
	...
	local bShouldKick = false; -- sorry I have no idea of how the fuck I'm gonna change this instead
	...
	if (type(player) == "Player") then
		if (!hook.Call("serverguard.PlayerBanned", nil, player, length, reason, admin)) then
			...
			-- why the fuck they didn't move it here???
			hook.Run( "serverguard.PostPlayerBanned", admin, player:SteamID(), reason, length );
		end;
