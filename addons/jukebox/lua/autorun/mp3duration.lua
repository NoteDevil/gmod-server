...
hook.Add("Medialib_ExtendQuery", "Medialib_MP3DurationEstimator", function(url, cbchain)
	if not string.EndsWith(url, ".mp3") then return end

	local function ParseDuration(err, data, cb)
		local dur = estimate_data(data._raw_fetcheddata)
		if dur then
			data.duration = dur
		end
