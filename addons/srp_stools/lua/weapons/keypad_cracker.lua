...
SWEP.Author = "Willox"
SWEP.Instructions = "ЛКМ — взломать кейпад"
SWEP.Contact = ""
SWEP.Purpose = ""
		...
		if SERVER then
			...
			timer.Create("KeyCrackSounds: "..self:EntIndex(), 1, self:GetCrackTime(), function()
				if IsValid(self) and self.IsCracking then
					self:EmitSound(self.KeyCrackSound, 70, 100)
...
if(CLIENT) then
	...
	net.Receive("KeypadCracker_Sparks", function()
		...
		if IsValid(ent) then
			...
			ent:EmitSound("buttons/combine_button7.wav", 70, 100)
		end
	end)
end
