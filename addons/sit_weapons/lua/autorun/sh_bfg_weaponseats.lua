...
hook.Add("CanPlayerEnterVehicle", "BFG_WeaponSeats_PlayerWeaponEnabler", function(ply, vehicle, role)
	if vehicle:GetNWBool( "Driver" ) then
		ply:SetNWBool("SeatWeapon_Usage_Allowed", false)
	else
		ply:SetNWBool("SeatWeapon_Usage_Allowed", true)
	end
...
net.Receive("WeaponSeats_ToggleCommand", function(len, ply)
	...
	if IsValid( vehicle ) and vehicle:GetNWBool( "Driver" ) then return end
...
-- 		draw.RoundedBox( 2, center.x, center.y, 4, 4, Color(255,255,255, 100) )
-- 	end
-- end)
