...
local function Sit(ply, pos, ang, parent, parentbone, func, exit)
	if parent and parent:GetPhysicsObject():IsMotionEnabled() then return end
	...
	if !tr.Hit then
		...
		vehicle:SetNetworkedBool( "saw", true )
...
hook.Add( "CanExitVehicle", "CustomExit",function( veh, ply )
	if veh:GetNetworkedBool( "saw" ) then return false end
end)
hook.Add( "Think", "CustomExit", function()
	for k,ply in pairs( player.GetAll() ) do
		if IsValid(ply) then
			...
			if veh:GetNetworkedBool( "saw" ) then
				...
				if ply:KeyDown( IN_USE ) then
					...
					if tr.Hit and IsValid(tr.Entity) and !tr.Entity:IsPlayer() and tr.Entity:GetPos():Distance( ply:GetPos() ) < 100 then
						if ply:KeyDown( IN_WALK ) then
							-- ply:PickupItem(  )
						elseif useWhiteList[ tr.Entity:GetClass() ] then
		...
		for k,v in pairs(ents.FindInSphere(EyeTrace.HitPos, 5)) do
			...
			and not v.PlayerSitOnPlayer
				...
				if sitting_disallow_on_me then
					ply:ChatPrint("Нельзя сидеть на игроках!")
					return
				end
