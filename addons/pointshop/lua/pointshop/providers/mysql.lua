...
local mysql_username = 'gmodrp' -- Your MySQL username.
local mysql_password = 'ohThisRoleplay' -- Your MySQL password.
local mysql_database = 'gmod_school' -- Your MySQL database.
...
function PROVIDER:GetData(ply, callback)
    ...
    qs = string.format(qs, ply:SteamID())
    local q = db:query(qs)
    ...
    function q:onSuccess(data)
        if #data > 0 then
		    ...
		    qs2 = string.format(qs2, ply:SteamID())
		    local q2 = db:query(qs2):start()
...
function PROVIDER:SetPoints(ply, points)
    ...
    qs = string.format(qs, ply:SteamID(), points or 0)
    local q = db:query(qs)
...
function PROVIDER:GiveItem(ply, item_id, data)
    ...
    qs = string.format(qs, ply:SteamID(), db:escape(util.TableToJSON(tmp)))
    local q = db:query(qs)
...
function PROVIDER:SetData(ply, points, items)
    ...
    qs = string.format(qs, ply:SteamID(), points or 0, db:escape(util.TableToJSON(items)))
    local q = db:query(qs)
