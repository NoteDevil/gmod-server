...
function ENT:Think()		
	...
	self.doorData = self.doorData or self.Door:getDoorData()
	if not self.Magick and self.doorData.owner == nil then self:Remove() end
	
	if self.Door.m_bLocked and !self.Locked then
		self.Locked = true
		self:AnimClose()
	end
	
	if not self.Door.m_bLocked and self.Locked then
		self.Locked = false
		self:AnimOpen()
	end
